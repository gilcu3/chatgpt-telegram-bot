...
| `STREAM`                            | Whether to stream responses. **Note**: incompatible, if enabled, with `N_CHOICES` higher than 1. In Claude mode, if `ENABLE_FUNCTIONS=true` and tools are configured, responses may fall back to non-streaming for tool-call handling.                                             | `true`                             |
| `TELEGRAM_NATIVE_STREAM`            | Experimental toggle for future Telegram native streaming transport. Currently logs intent and keeps legacy edit-based streaming behavior for compatibility.                                                                                                                                | `false`                            |
